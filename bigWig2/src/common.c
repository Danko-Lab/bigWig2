...
if (size != 0 && fread(buf, size, 1, file) != 1)
    {
    if (ferror(file))
	errAbort("Error reading %"PRIdMAX" bytes: %s", (intmax_t)size, strerror(ferror(file)));
    else
	errAbort("End of file reading %"PRIdMAX" bytes", (intmax_t)size);
    }
}
...
while (size > 0)
    ...
    if (actualSize < 0)
	errnoAbort("Error reading %"PRIdMAX" bytes", (intmax_t)size);
    if (actualSize == 0)
	errAbort("End of file reading %"PRIuMAX" bytes (got %"PRIdMAX")", (uintmax_t)size, (intmax_t)actualSize);
    cbuf += actualSize;
    size -= actualSize;
    }
}
...
if (ret < 0)
    errnoAbort("lseek(%d, %"PRIdMAX", %s (%d)) failed", fd, (intmax_t)offset,
...
for (oldEl = inList; oldEl != NULL; oldEl = nextOld)
    ...
    if (nextOld != NULL && nextOld->offset < oldEl->offset)
        errAbort("Unsorted inList in fileOffsetSizeMerge %"PRIu64" %"PRIu64"", oldEl->offset, nextOld->offset);
...
if (slen > bufSize-1)
    errAbort("buffer overflow, size %"PRIdMAX", string size: %"PRIdMAX"", (intmax_t)bufSize, (intmax_t)slen);
strcpy(buf, src);
}
...
{
if (n > bufSize-1)
    errAbort("buffer overflow, size %"PRIdMAX", substring size: %"PRIdMAX"", (intmax_t)bufSize, (intmax_t)n);
...
if (blen+slen > bufSize-1)
    errAbort("buffer overflow, size %"PRIdMAX", new string size: %"PRIdMAX"", (intmax_t)bufSize, (intmax_t)(blen+slen));
strcat(buf, src);
}
...
size_t blen = strlen(buf);
if (blen+n > bufSize-1)
    errAbort("buffer overflow, size %"PRIdMAX", new string size: %"PRIdMAX"", (intmax_t)bufSize, (intmax_t)(blen+n));
size_t slen = strlen(src);
if (slen > n)
    slen = n;
strncat(buf, src, n);
