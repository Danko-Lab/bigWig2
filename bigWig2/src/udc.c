...
verbose(2, "reading remote data - %d bytes at %"PRIu64" - on %s\n", size, offset, url);
...
if (ferror(f))
    {
    warn("udcDataViaLocal failed to fetch %d bytes at %"PRIu64"", size, offset);
...
* error.  Typically will be called with size in the 8k - 64k range. */
{
verbose(2, "slow reading remote data - %d bytes at %"PRIu64" - on %s\n", size, offset, url);
sleep1000(500);
...
for (i=0; i<size; i += step)
    ...
    if (ferror(f))
	{
	warn("udcDataViaSlow failed to fetch %d bytes at %"PRIu64"", size, offset);
...
if (bits != NULL)
	...
	{
	verbose(2, "removing stale version (%"PRIu64"! = %"PRIdMAX" or %"PRIu64"! = %"PRIu64" or %s doesn't exist), "
		"new version %d\n",
		bits->remoteUpdate, (intmax_t)file->updateTime, bits->fileSize, file->size,
...
if (endPos > startPos)
	...
	errAbort("unable to fetch %"PRIu64" bytes from %s @%"PRIu64" (got %d bytes)",
...
if (maxSize != 0 && size > maxSize)
    errAbort("%s is %"PRIdMAX" bytes, but maxSize to udcFileReadAll is %"PRIdMAX"",
    	url, (intmax_t)size, (intmax_t)maxSize);
